...
def compute_PIs(X,Y,X1,alpha,fit_muh_fun, weights_oracle, dataset, bias, run_effective_sample_size, weights_lr, weights_rf, K_vals):
    ...
    #     K = 10
        ...
        ## [oracle, logistic_regression_estimated, random_forest_estimated]
        weights_full_list = [weights_oracle, weights_lr, weights_rf]
        weights_normalized_list = []

        y_upper_JAW_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)] 
        y_lower_JAW_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)]

        y_upper_wCV_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)] 
        y_lower_wCV_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)]

        y_upper_JAW_KLOO_rep_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)] 
        y_lower_JAW_KLOO_rep_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)]
        
        y_upper_JAW_KLOO_det_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)] 
        y_lower_JAW_KLOO_det_list = [np.zeros(n1), np.zeros(n1), np.zeros(n1)]

        JAW_KLOO_inds = [[],[],[]]

        ## Calculate normalized likelihood ratio weights
        for weight_type in range(0, 3):
            weights_full = weights_full_list[weight_type]
            ...
            weights_normalized_list.append(weights_normalized)




        for weight_type in range(0, 3):
            weights_normalized = weights_normalized_list[weight_type]
            for j in range(0, n1):
                ...
                y_upper_wCV_list[weight_type][j] = weighted_quantile(unweighted_upper_vals_CV[:, j], weights_normalized[:, j], 1 - alpha)
                y_lower_wCV_list[weight_type][j] = weighted_quantile(unweighted_lower_vals_CV[:, j], weights_normalized[:, j], alpha)

                ## K LOO sampling with replacement
                weights_normalized_j_cum = np.cumsum(weights_normalized[:, j])

                while (len(set(JAW_KLOO_inds[weight_type])) < K):
                    JAW_KLOO_inds[weight_type].append(random.choices(list(range(0, n+1)), cum_weights = weights_normalized_j_cum)[0])

                JAW_KLOO_inds_weight_type = np.array(JAW_KLOO_inds[weight_type])

                upper_vals_JAW_KLOO_all = unweighted_upper_vals[:, j][JAW_KLOO_inds_weight_type]
                lower_vals_JAW_KLOO_all = unweighted_lower_vals[:, j][JAW_KLOO_inds_weight_type]

                upper_vals_JAW_KLOO_unique = np.unique(unweighted_upper_vals[:, j][JAW_KLOO_inds_weight_type], return_counts=True)
                lower_vals_JAW_KLOO_unique = np.unique(unweighted_lower_vals[:, j][JAW_KLOO_inds_weight_type], return_counts=True)

                y_upper_JAW_KLOO_rep_list[weight_type][j] = weighted_quantile(upper_vals_JAW_KLOO_unique[0], upper_vals_JAW_KLOO_unique[1]/np.sum(upper_vals_JAW_KLOO_unique[1]), 1 - alpha)
                y_lower_JAW_KLOO_rep_list[weight_type][j] = weighted_quantile(lower_vals_JAW_KLOO_unique[0], lower_vals_JAW_KLOO_unique[1]/np.sum(lower_vals_JAW_KLOO_unique[1]), alpha)
                
                
                ## K LOO deterministic
                inds_K_largest = np.argpartition(weights_normalized[:, j], -K)[-K:]
                K_LOO_det_norm_weights = weights_normalized[:, j][inds_K_largest]/np.sum(weights_normalized[:, j][inds_K_largest])
                y_upper_JAW_KLOO_det_list[weight_type][j] = weighted_quantile(unweighted_upper_vals[:, j][inds_K_largest], K_LOO_det_norm_weights, 1 - alpha)
                y_lower_JAW_KLOO_det_list[weight_type][j] = weighted_quantile(unweighted_lower_vals[:, j][inds_K_largest], K_LOO_det_norm_weights, alpha)
        ...
        PIs_dict['JAW_KLOO_oracle_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_JAW_KLOO_rep_list[0], \
                    y_upper_JAW_KLOO_rep_list[0]],\
                       columns = ['lower','upper'])
        
        PIs_dict['JAW_KLOO_lr_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_JAW_KLOO_rep_list[1], \
                    y_upper_JAW_KLOO_rep_list[1]],\
                       columns = ['lower','upper'])
        
        PIs_dict['JAW_KLOO_rf_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_JAW_KLOO_rep_list[2], \
                    y_upper_JAW_KLOO_rep_list[2]],\
                       columns = ['lower','upper'])
        
        PIs_dict['JAW_KLOO_oracle_det_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_JAW_KLOO_det_list[0], \
                    y_upper_JAW_KLOO_det_list[0]],\
                       columns = ['lower','upper'])
        
        PIs_dict['JAW_KLOO_lr_det_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_JAW_KLOO_det_list[1], \
                    y_upper_JAW_KLOO_det_list[1]],\
                       columns = ['lower','upper'])
        
        PIs_dict['JAW_KLOO_rf_det_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_JAW_KLOO_det_list[2], \
                    y_upper_JAW_KLOO_det_list[2]],\
                       columns = ['lower','upper'])
        
        PIs_dict['wCV_oracle_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_wCV_list[0], \
                    y_upper_wCV_list[0]],\
                       columns = ['lower','upper'])
        
        PIs_dict['wCV_lr_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_wCV_list[1], \
                    y_upper_wCV_list[1]],\
                       columns = ['lower','upper'])
        
        PIs_dict['wCV_rf_K' + str(K)] = pd.DataFrame(\
                np.c_[y_lower_wCV_list[2], \
                    y_upper_wCV_list[2]],\
                       columns = ['lower','upper'])
...
def prob_interval_sorted_unweighted(PVs_itrial, y_pred_lower, y_pred_upper, test_pt, method, n_eff):
    
    
    idx_low = 0
    train_scores_lower = np.array(np.sort(PVs_itrial[PVs_itrial['method']==method]['lower' + str(test_pt)]))
    idx_up = 0
    train_scores_upper = np.array(np.sort(PVs_itrial[PVs_itrial['method']==method]['upper' + str(test_pt)]))
    if (n_eff < len(train_scores_lower)):
        n_eff_indices = random.sample(range(0,len(train_scores_lower)), n_eff) 
        train_scores_lower = train_scores_lower[n_eff_indices]
        train_scores_upper = train_scores_upper[n_eff_indices]
...
def calculate_AUC(PVs_path, dataset, bias, mu, effective_SS = False, num_taus = 20):
    
    PVs = pd.read_csv(PVs_path)
    
    if (effective_SS):
        ### CHECK WHETHER THESE IMPORTANCE WEIGHTS ARE NORMALIZED OR NOT, SHOULD NOT BE
#         importance_weights = np.array(PVs_itrial[PVs_itrial['method']=='weights_JAW_train']['lower' + str(test_pt)])
#         importance_weights = importance_weights * np.sum(importance_weights)
#         n_eff = int(np.linalg.norm(importance_weights, ord=1)**2 / np.linalg.norm(importance_weights, ord=2)**2)
        n_eff = 50
    ...
    split_probs_ALL = []
    CV_probs_ALL = []
    jackknife_plus_probs_effSS_ALL = []
    split_probs_effSS_ALL = []
    CV_probs_effSS_ALL = []
    JAW_probs_effSS_ALL = []
    correctness_ALL = []
    ...
    for itrial in tqdm.tqdm(set(PVs['itrial'])):
        ...
        split_probs_itrial = []
        CV_probs_itrial = []
        jackknife_plus_probs_effSS_itrial = []
        split_probs_effSS_itrial = []
        CV_probs_effSS_itrial = []
        JAW_probs_itrial = []
        correctness_itrial = []
        ...
        for tau in taus:
            ...
            for test_pt in range(0, np.max(PVs['itrial']) + 1):
                ...
                jackknife_plus_effSS_probs.append(prob_interval_sorted_unweighted(PVs_itrial, y_pred_lower, y_pred_upper, test_pt, 'jackknife+_sorted', n_eff))
                split_effSS_probs.append(prob_interval_sorted_unweighted(PVs_itrial, y_pred_lower, y_pred_upper, test_pt, 'split', n_eff))
                CV_effSS_probs.append(prob_interval_sorted_unweighted(PVs_itrial, y_pred_lower, y_pred_upper, test_pt, 'CV+', n_eff))
            ...
            jackknife_plus_probs_effSS_itrial.append(jackknife_plus_effSS_probs)
            split_probs_effSS_itrial.append(split_effSS_probs)
            CV_probs_effSS_itrial.append(CV_effSS_probs)
        ...
        jackknife_plus_probs_effSS_ALL.append(jackknife_plus_probs_effSS_itrial)
        split_probs_effSS_ALL.append(split_probs_effSS_itrial)
        CV_probs_effSS_ALL.append(CV_probs_effSS_itrial)
    ...
    AUCs_split_ALL = []
    AUCs_CV_ALL = []
    AUCs_jackknife_plus_effSS_ALL = []
    AUCs_split_effSS_ALL = []
    AUCs_CV_effSS_ALL = []
    AUCs_JAW_ALL = []
    ...
    for itrial in set(PVs['itrial']):
        ...
        AUCs_split_itrial = []
        AUCs_CV_itrial = []
        AUCs_jackknife_plus_effSS_itrial = []
        AUCs_split_effSS_itrial = []
        AUCs_CV_effSS_itrial = []
        AUCs_JAW_itrial = []
        ...
        jackknife_plus_probs_effSS_itrial = jackknife_plus_probs_effSS_ALL[itrial]
        split_probs_effSS_itrial = split_probs_effSS_ALL[itrial]
        CV_probs_effSS_itrial = CV_probs_effSS_ALL[itrial]
        ...
        for i in range(0, num_taus):
            ...
            AUCs_split_effSS_itrial.append(roc_auc_score(correctness, split_probs_effSS_itrial[i]))
            AUCs_CV_effSS_itrial.append(roc_auc_score(correctness, CV_probs_effSS_itrial[i]))
            AUCs_jackknife_plus_effSS_itrial.append(roc_auc_score(correctness, jackknife_plus_probs_effSS_itrial[i]))
        ...
        AUCs_jackknife_plus_effSS_ALL.append(AUCs_jackknife_plus_effSS_itrial)
        AUCs_split_effSS_ALL.append(AUCs_split_effSS_itrial)
        AUCs_CV_effSS_ALL.append(AUCs_CV_effSS_itrial)
    ...
    AUC_results = pd.DataFrame({'taus': taus, 'split': np.mean(AUCs_split_ALL, axis = 0), 
        ...
        'jackknife+_effSS':np.mean(AUCs_jackknife_plus_effSS_ALL, axis = 0),
        'CV+_effSS':np.mean(AUCs_CV_effSS_ALL, axis = 0),
        'JAW_effSS':np.mean(AUCs_JAW_effSS_ALL, axis = 0)})
    
    AUC_results_path = './results/JAWE_error_assessment/' + str(date.today()) + '_AUC_' + dataset + '_' + mu + '_' + str(num_trials) + '_Trials_effSS.csv'
...
def plot_AUC(AUC_results_path, dataset, mu, num_trials, save_figure = True):
    ...
    plt.plot(AUC_results['taus'], AUC_results['JAW'], label = "JAW", color= 'C0', linewidth=4)
    plt.plot(AUC_results['taus'], AUC_results['jackknife+_effSS'], label = "Jackknife+", color = 'C2', linewidth=2, linestyle='dashed')
    plt.plot(AUC_results['taus'], AUC_results['CV+_effSS'], label = "CV+", color= 'C4', linewidth=2, linestyle='dashed')
    plt.plot(AUC_results['taus'], AUC_results['split_effSS'], label = "Split conformal", color = 'C5', linewidth=2, linestyle='dashed')
    ...
    if (save_figure):
        plt.savefig('./results_figures/fig5and8_ErrorAssessAUC/' + str(date.today()) + '_AUC_' + str(dataset) + '_' + mu + '_' + str(num_trials) + 'Trials_effSS.png',dpi=200,bbox_inches='tight')
    plt.show()
