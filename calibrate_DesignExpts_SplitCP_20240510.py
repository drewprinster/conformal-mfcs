...
def compute_w_ptest_split_active_replacement(cal_test_vals_mat, n_step_curr):
    ...
    if (n_step_curr == 1):
        ...
        return cal_test_vals_mat[-1] ## Added [-1] on 20240223
...
#             print("w_n1xy_adjusted shape : ", np.shape(w_n1xy_adjusted))
#             print("idx_include shape : ", np.shape(idx_include))
#             print("Xaug_n1xp shape :", np.shape(Xaug_n1xp))
#             print("ytrain_n shape : ", np.shape(ytrain_n))
...
class JAW_FCS_ACTIVE(ABC):
    ...
    def compute_PIs_active_design(self, Xtrain_nxp, ytrain_n, Xtest_1xp, ytest_n1, Xtrain_split, Xcal_split, ytrain_split, ycal_split, Xtest_n1xp_split, ytest_n1_split, Xpool_split, w_split_mus_prev_steps, w_LOO_mus_prev_steps, method_names, weight_bounds, n_step_curr, X_dataset, weight_adj_depth_maxes = [1,2], tilt_factor = 1/10, bandwidth = 1.0, alpha: float = 0.1, K_vals = [8, 12, 16, 24, 32, 48], n_train_initial=100, n_dataset = None, replacement=True):
        # , add_to_cal=True
        if (self.p != Xtrain_nxp.shape[1]):
            raise ValueError('Feature dimension {} differs from provided Xuniv_uxp {}'.format(
                Xtrain_nxp.shape[1], self.Xuniv_uxp.shape))
...
#         w_split_mus_prev_and_curr_steps.append(deepcopy(self.model))
        ...
        w_split_MAT_all_steps = np.zeros((n_step_curr, n_cal + 1))
        pool_weights_totals_prev_steps = np.zeros(n_step_curr)
        for i, muh_split_curr in enumerate(w_split_mus_prev_and_curr_steps):
#             ## Compute weights for cal and test points
#             y_muh_curr = muh_split_curr.predict(np.r_[Xcal_split,Xtest_n1xp_split])
#             w_split_MAT_all_steps[i] = np.exp(y_muh_curr * tilt_factor)
            
#             ## Compute total weights for pool using previous 
#             y_pool_muh_curr = muh_split_curr.predict(Xpool_split)
#             pool_weights_totals_prev_steps[i] = np.sum(np.exp(y_pool_muh_curr * tilt_factor))
            ...
            exp_y_muh_curr = np.exp(y_muh_curr * tilt_factor)
            ...
            exp_y_muh_curr_pool = np.exp(y_muh_curr_pool * tilt_factor)
...
#             print("wsplit_quantiles : ", wsplit_quantiles)
        ...
        if ('JAW-FCS' in method_names or 'JAW-SCS' in method_names or 'jackknife+' in method_names):
            ###############################
            # JAW FCS & SCS
            ###############################
    ...
    #         n_train_initial = n
            resids_LOO = np.zeros(n_train_initial)
            muh_LOO_vals_testpoint = np.zeros((n_train_initial,n1))


            unnormalized_weights_JAW_FCS = np.zeros((n_train_initial + 1, n1))
            unnormalized_weights_JAW_SCS = np.zeros((n_train_initial + 1, n1))

            w_LOO_mat_curr_step = np.zeros((n_train_initial+1, n1))

            print("n, n_train_initial : ", n, n_train_initial)
    #         for i in range(n):
            for i in range(n_train_initial):
                ## Create LOO X and y data
                Xi_LOO_n_minus_1xp = np.vstack([Xaug_n1xp[: i], Xaug_n1xp[i + 1 : n]]) ## LOO training data inputs
                yi_LOO_train_n = np.concatenate((ytrain_n[: i], ytrain_n[i + 1 : n])) ## LOO training data outputs
    ...
    #             density_i = density_i / np.sum(density_i)
                ...
                ptest_i = np.exp(std_i_LOO**2) / np.sum(np.exp(var_preds_pool_all_steps[-1]))        
                ptrain_i = 1 / len(var_preds_pool_all_steps[-1])

                ptest_testpoint_i = np.exp(std_LOO_vals_testpoint_i**2) / np.sum(np.exp(var_preds_pool_all_steps[-1]))
                ptrain_testpoint_i = 1 / len(var_preds_pool_all_steps[-1])


#                 unnormalized_weights_JAW_FCS[i] = (ptest_i / ptrain_i) * (ptest_testpoint_i / ptrain_testpoint_i)
                unnormalized_weights_JAW_FCS[i] = (ptest_i) * (ptest_testpoint_i)
    
    #             w_LOO_mat_curr_step[i] = (ptest_i / ptrain_i) * (ptest_testpoint_i / ptrain_testpoint_i)

                w_LOO_mat_curr_step[i] = (ptest_testpoint_i)**2 ##/ ptrain_testpoint_i
    ...
    #         density_testpoint = density_testpoint / np.sum(density_testpoint)
            ptest_testpoint = np.exp(std_LOO_vals_testpoint_i**2) / np.sum(np.exp(var_preds_pool_all_steps[-1]))
            ptrain_testpoint = 1 / len(var_preds_pool_all_steps[-1])
    #         unnormalized_weights_JAW_FCS[-1] = (np.exp(std_test**2) / ptrain_testpoint)**2
            unnormalized_weights_JAW_FCS[-1] = (ptest_testpoint)**2 ##/ ptrain_testpoint_i

            w_LOO_mat_curr_step[-1] = (ptest_testpoint)**2 ##/ ptrain_testpoint
            ...
            ptest_train_test = std_train_test**2 # pred_train_test
    #         SCS_weights_vec = self.get_lrs(Xaug_n1xp, ytrain_n, lmbda)
    ...
    #         density_train_test = density_train_test / np.sum(density_train_test)
            density_train_test = 1 / len(var_preds_pool_all_steps[-1])
            SCS_weights_vec = ptest_train_test / density_train_test
            weights_normalized_JAW_SCS = np.zeros((n_train_initial + 1, n1))
            sum_train_weights = np.sum(SCS_weights_vec[0:n_train_initial])
            for j in range(0, n1):
                for i in range(0, n_train_initial + 1):
                    if (i < n_train_initial):
                        weights_normalized_JAW_SCS[i, j] = SCS_weights_vec[i] / (sum_train_weights + SCS_weights_vec[n_train_initial + j])
                    else:
                        weights_normalized_JAW_SCS[i, j] = SCS_weights_vec[n_train_initial+j] / (sum_train_weights + SCS_weights_vec[n_train_initial + j])
            ...
            weights_normalized_JAW_FCS = np.zeros((n_train_initial + 1, n1))
            for j in range(0, n1):
                weights_normalized_JAW_FCS[:,j] = unnormalized_weights_JAW_FCS[:,j] / np.sum(unnormalized_weights_JAW_FCS[:,j])
            ...
            y_lower_Jplus = np.zeros(n1)
            y_upper_Jplus = np.zeros(n1)
            uniform_weights = np.ones(n_train_initial+1) / (n_train_initial+1)
            for j in range(0, n1):
                y_lower_JAW_FCS[j] = weighted_quantile(unweighted_lower_vals[:, j], weights_normalized_JAW_FCS[:, j], alpha)
                y_upper_JAW_FCS[j] = weighted_quantile(unweighted_upper_vals[:, j], weights_normalized_JAW_FCS[:, j], 1 - alpha)
                y_lower_JAW_SCS[j] = weighted_quantile(unweighted_lower_vals[:, j], weights_normalized_JAW_SCS[:, j], alpha)
        ...
        if ('JAW-FCS' in method_names or 'JAW-SCS' in method_names or 'jackknife+' in method_names):
            PIs_dict['JAW-FCS'] = pd.DataFrame(np.c_[y_lower_JAW_FCS, \
                                  y_upper_JAW_FCS], columns = ['lower','upper'])
            
            PIs_dict['JAW-SCS'] = pd.DataFrame(np.c_[y_lower_JAW_SCS, \
                                  y_upper_JAW_SCS], columns = ['lower','upper'])
            
            PIs_dict['jackknife+'] = pd.DataFrame(np.c_[y_lower_Jplus, \
                                     y_upper_Jplus], columns = ['lower','upper'])
        
            
        
        return PIs_dict, w_split_mus_prev_steps, w_LOO_mus_prev_steps
