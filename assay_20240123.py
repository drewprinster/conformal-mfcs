...
from calibrate_20240128_FIXED import get_invcov_dot_xt
...
def get_training_and_designed_data(data, n, gamma, lmbda, seed: int = None, n1 = 1, replace = True):
    """
    ...
    test_idx = rng.choice(data.n, n1, replace = replace, p=punnorm_n / Z if lmbda else None)
...
## Drew added this function for allowing multistep experiments
def sample_new_designed_data(data, Xtrain_nxp, ytrain_n, n, gamma, lmbda, seed: int = None, n1 = 1, replace = True):
    """
    Sample sample one designed protein (w/ ground-truth label) according to design algorithm, using input training data.
    ...
    :param lmbda: float, inverse temperature of design algorithm in Eq. 6, {0, 2, 4, 6} in main paper
    :param seed: int, random seed
    :return: numpy arrays of designed sequence, label, and prediction
    """
    rng = np.random.default_rng(seed)
...
def get_training_and_designed_data_my_muh(data, n, gamma, lmbda, seed: int = None, n1 = 1, replace = True, muh_name='NN'):
    """
    Sample training data uniformly at random from combinatorially complete data set (Poelwijk et al. 2019),
    and sample one designed protein (w/ ground-truth label) according to design algorithm in Eq. 6 of main paper.
    ...
    # get random data for split conformal
    idx_split = np.random.permutation(n)
    n_half = int(np.floor(n/2))
    idx_train_split, idx_cal_split = idx_split[:n_half], idx_split[n_half:]
    Xtrain_split = Xtrain_nxp[idx_train_split]
    Xcal_split = Xtrain_nxp[idx_cal_split]
    ytrain_split = ytrain_n[idx_train_split]
    ycal_split = ytrain_n[idx_cal_split]
    
    if (muh_name=='NN'):
        muh_fun = MLPRegressor(solver='lbfgs',activation='logistic')
        muh_fun_split = MLPRegressor(solver='lbfgs',activation='logistic')
    elif (muh_name=='RF'):
        muh_fun = RandomForestRegressor(n_estimators=20,criterion='absolute_error')
        muh_fun_split = RandomForestRegressor(n_estimators=20,criterion='absolute_error')
        
        
    muh_fun.fit(Xtrain_nxp, ytrain_n)
    predall_n = muh_fun.predict(data.X_nxp)
...
#     predall_n = data.X_nxp.dot(beta_p)
    ...
    # get noisy measurement and model prediction for designed sequence
    ytest_n1 = data.get_measurements(test_idx)
    pred_train_test = muh_fun.predict(np.vstack([Xtrain_nxp, Xtest_n1xp]))
    
    
       
    muh_fun_split.fit(Xtrain_split, ytrain_split)
    predall_n_split = muh_fun_split.predict(data.X_nxp)
    
    punnorm_n_split = np.exp(lmbda * predall_n_split)
    Z_split = np.sum(punnorm_n_split)
    
    test_idx_split = rng.choice(data.n, n1, replace = replace, p=punnorm_n_split / Z_split if lmbda else None)
    Xtest_n1xp_split = data.X_nxp[test_idx_split]
    
    ytest_n1_split = data.get_measurements(test_idx_split)
    pred_cal_test_split = muh_fun_split.predict(np.vstack([Xcal_split, Xtest_n1xp]))
    
    
    
    return Xtrain_nxp, ytrain_n, Xtest_n1xp, ytest_n1, pred_train_test, Xtrain_split, Xcal_split, ytrain_split, ycal_split, Xtest_n1xp_split, ytest_n1_split, pred_cal_test_split



def get_data_and_params_IFs(data, n, gamma, lmbda, seed: int = None, n1 = 1, replace = True, muh_name='NN', L2_lambda = 5, num_hidden = 25):
    """
    Sample training data uniformly at random from combinatorially complete data set (Poelwijk et al. 2019),
    and sample one designed protein (w/ ground-truth label) according to design algorithm in Eq. 6 of main paper.
    ...
    predall_n = model.predict(params, data.X_nxp) ## Same as muh_vals_testpoint
    ...
    # get noisy measurement and model prediction for designed sequence
    ytest_n1 = data.get_measurements(test_idx)
    pred_train_test = model.predict(params, np.vstack([Xtrain_nxp, Xtest_n1xp]))
    ...
    params_split = bayesnn.train(objective_split, init_params_split, Xtrain_split, ytrain_split, config_split, weights_split)
       
    predall_n_split = model_split.predict(params_split, data.X_nxp)
    ...
    ytest_n1_split = data.get_measurements(test_idx_split)
    pred_cal_test_split = model_split.predict(params_split, np.vstack([Xcal_split, Xtest_n1xp]))
    
    
    
    return Xtrain_nxp, ytrain_n, Xtest_n1xp, ytest_n1, pred_train_test, Xtrain_split, Xcal_split, ytrain_split, ycal_split, Xtest_n1xp_split, ytest_n1_split, pred_cal_test_split, params, params_split
